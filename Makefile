...
ifneq ($(LUAROCKS_VER),'luarocks 3')
	luarocks install rockspec/orange-dev-1.0-0.rockspec --tree=deps --only-deps --local
else
	luarocks install --lua-dir=/usr/local/openresty/luajit rockspec/orange-dev-1.0-0.rockspec --tree=deps --only-deps --local
endif
