(function(L) {
    var _this = null;
    L.ConsulBalancer = L.ConsulBalancer || {};
    _this = L.ConsulBalancer = {
        data: {},

        init: function() {
            L.Common.loadConfigs("consul_balancer", _this, true);
            _this.initEvents();
        },

        initEvents: function() {
            var op_type = "consul_balancer";
            ...
            _this.initClearBtnEvent(); //清除统计按钮事件
                            ...
                            if (result.success) {
                                $.ajax({
                                    ...
                                    success: function(result) {
                                        if (result.success) {
                                            // 重新渲染
                                            L.Common.loadConfigs("consul_balancer", context, false, function() {
                            ...
                            if (result.success == true) {
                                $.ajax({
                                    ...
                                    success: function(result) {
                                        if (result.success) {
                                            //重新渲染
                                            L.Common.loadConfigs("consul_balancer", context);
                                            return true;
                                        } else {
        ...
        initUpstreamClickEvent: function(context) {
            $(document).on("click", ".selector-item", function() {
                ...
                var now_state = $("#stat-btn").attr("data-show");
                //if (now_state == "true") {
                _this.loadStat();
                /*} else {
                ...
                var d = dialog({
                    ...
                    button: [{
                        ...
                        callback: function() {
                            ...
                            if (result.success == true) {
                                $.ajax({
                                    url: '/consul_balancer/selectors/' + selector_id + "/rules",
                                    type: 'post',
                                    data: {
                                    ...
                                    success: function(result) {
                                        if (result.success) {
                                            ...
                                            _this.loadHosts(context, selector_id);
                                            // 刷新缓存
                                            L.Common.refreshConfigs("consul_balancer", context);
                                        } else {
        ...
        buildUpstream: function() {
            ...
            var service = $("#selector-service").val();
            if (!service) {
                result.data = "服务名不能为空";
                return result;
            }
            ...
            result.data.log_consul = ($("#selector-log-consul").val() === "true");
        ...
        loadStat: function() {
            ...
            var name = $(".selector-item.info-element.selected-selector").attr("data-name");
            $.ajax({
                url: '/consul_balancer/stat',
                type: 'get',
                cache: false,
                data: {
                    service: name
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        L.Common.showTipDialog("错误提示", "查询consul统计请求发生错误");
                    }
                },
                error: function() {
                    L.Common.showTipDialog("提示", "查询consul统计请求发生异常");
                }
            });
        },
        ...
        clearStat: function() {
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        L.Common.showTipDialog("提示", "已清除consul统计数据");
                        _this.loadStat();
                    } else {
                        L.Common.showTipDialog("错误提示", "清除consul统计请求发生错误");
                    }
                },
                error: function() {
                    L.Common.showTipDialog("提示", "清除consul统计请求发生异常");
                }
            });
        },
    };
